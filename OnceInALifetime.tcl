# =============================================================================
  # OnceInALifetime.tcl
  # Jeff Godfrey, March 2004

  # tcl/tk version of the card game "Once in A Lifetime"

  # Idea taken from game found at http://www.alphalink.com.au/~gregr/once.htm
  ...
  proc reset {} {
      ...
      foreach suit {s d c h} {
          foreach value {a k q j t 9 8 7 6 5 4 3 2} {
              lappend ::data(cards) "$value$suit"
          }
      }

      # --- delete all the card items
      eval .c1 delete [.c1 find withtag card]

      # --- reset and rewire the deck item
      .c1 itemconfigure deck -image ::img::back
      .c1 bind deck <ButtonPress-1> dealNextRow

      # --- reset the discard pile image
      .c1 itemconfigure discard -image ::img::marker
  }
  ...
  proc buildUI {} {
      ...
      .m.game add command -label "New Game" -underline 0 -command newGame
      .m.game add checkbutton -label "Show Deal Hints" -underline 0 \
          -variable ::data(hints) -command checkGameStatus
      .m.game add separator
      .m.game add command -label "Exit"  -underline 0 -command exit
      .m.help add command -label "Help"  -underline 0 -command help
      .m.help add separator
      .m.help add command -label "About" -underline 0 -command about

      canvas .c1 -width 600 -height 450 -borderwidth 0 -highlightthickness 0 \
         -bg darkgreen
      label .l1 -textvariable ::data(cardCount)
      pack .c1
      pack .l1 -expand 1 -fill x

      # --- add the deck item
      .c1 create image 490 310 -image ::img::back -anchor nw -tags deck
      .c1 bind deck <ButtonPress-1> dealNextRow

      # --- add the deal hint item
      #     position it just below the deck and hide it for now
      .c1 create rectangle 486 306 564 409 -fill red -outline red \
         -state hidden -tags hint
      .c1 lower hint deck

      # --- add the discard marker
      .c1 create image 490 110 -image ::img::marker -anchor nw -tags discard

      # --- add the 4 card pile markers
      for {set i 1} {$i <= 4} {incr i} {
          set xStart [expr {40 + (100 * ($i - 1))}]
         .c1 create image $xStart 10 -image ::img::marker -anchor nw \
             -tags [list marker col$i]
     }

     set ::data(title) "Once In A Lifetime"
     wm title . $::data(title)
     wm resizable . 0 0
 }
 ...
 proc dealNextRow {} {
     for {set i 1} {$i <= 4} {incr i} {
         # --- count all the cards in the current column
         set count [llength [.c1 find withtag col$i&&card]]
         set xPos [expr {40 + (100 * ($i - 1))}]
         set yPos [expr {10 + (25 * $count)}]
         set card [getNextCard]
         set id [.c1 create image $xPos $yPos -image ::img::$card \
             -anchor nw -tags [list $card col$i card]]
     }

     # --- if no more cards
     #        change the deck image to the marker
     #        unwire the "deal" binding
     if {![llength $::data(cards)]} {
         .c1 itemconfigure deck -image ::img::marker
         .c1 bind deck <ButtonPress-1> {}
     }
     set ::data(cardCount) "Cards remaining:  [llength $::data(cards)]"
     checkGameStatus
 }

 proc checkGameStatus {} {
     if {$::data(hints) && ![checkForLegalMove] && [llength $::data(cards)]} {
         .c1 itemconfigure hint -state normal
     } else {
         .c1 itemconfigure hint -state hidden
     }
     checkForWin
     checkForGameOver
     updateBindings
 ...
 proc updateBindings {} {
     ...
     foreach id [.c1 find withtag card] {
         set card [lindex [.c1 gettags $id] 0]
         .c1 bind $card <ButtonPress-1>   {}
         .c1 bind $card <B1-Motion>       {}
         .c1 bind $card <ButtonRelease-1> {}
         .c1 bind $card <Double-Button-1> {}
     }

     # --- add bindings to only the top cards
     foreach card [getTopCards] {
         .c1 bind $card <ButtonPress-1>   [list dragStart $card %x %y]
         .c1 bind $card <B1-Motion>       [list dragging $card %x %y]
         .c1 bind $card <ButtonRelease-1> [list dragEnd $card %x %y]
         .c1 bind $card <Double-Button-1> [list removeCard $card]
     }
 }

 proc removeCard {id} {
     set ::data(orgCoords) [.c1 coords $id]
     set compareList [getTopCards]
     set me [lindex [.c1 gettags $id] 0]
     lassign [cardInfo $me] meValue meSuit

     foreach card $compareList {
         if {$card ne $me} {
             lassign [cardInfo $card] thisValue thisSuit
             if {$thisSuit eq $meSuit && $thisValue > $meValue} {
                 .c1 delete $me
                 .c1 itemconfigure discard -image ::img::$me
                 break
             }
         }
     }
 ...
 proc checkForLegalMove {} {
     set cardList [getTopCards]
     foreach card $cardList {
         lassign [cardInfo $card] thisValue thisSuit
         lappend suitList $thisSuit
     }
     if {[llength $suitList] == [llength [lsort -uniq $suitList]]} {
         return 0
     } else {
         return 1
     }
 ...
 proc checkForWin {} {
     if {[llength [.c1 find withtag card]] != 4} {return}
     if {[llength [getTopCards]] != 4} {return}
     foreach card $::data(cards) {
         lassign [cardInfo $card] value suit
         if {$value != 14} {return}
     }
 ...
 proc checkForGameOver {} {
     if {[llength $::data(cards)]} {return}
     if {[llength [getTopCards]] != 4} {return}
     if {![checkForLegalMove]} {
         set resp [tk_messageBox \
             -title $::data(title) \
 ...
 proc cardInfo {card} {
     lassign [split $card ""] value suit
     if {$value eq "t"} {set value 10}
     if {$value eq "j"} {set value 11}
     if {$value eq "q"} {set value 12}
     if {$value eq "k"} {set value 13}
     if {$value eq "a"} {set value 14}
     return [list $value $suit]
 }

 proc getTopCards {} {
     foreach colName [list col1 col2 col3 col4] {
         set topID [lindex [.c1 find withtag $colName&&card] end]
         if {[llength $topID]} {
             set card  [lindex [.c1 gettags $topID] 0]
 ...
 proc dragging {id x y} {
     .c1 move $id [expr {$x - $::data(xLoc)}] [expr {$y - $::data(yLoc)}]
     set ::data(xLoc) $x
     set ::data(yLoc) $y
 }

 proc dragEnd {id x y} {
     set tagList [eval .c1 find overlapping [.c1 bbox $id]]
     if {[llength $tagList] == 2} {
         foreach tag $tagList {
             set theTags [.c1 gettags $tag]
             if {[lsearch $theTags "marker"] >= 0} {
                 set newCol [lindex $theTags 1]
                 set card   [lindex [.c1 gettags $id] 0]
                 .c1 coords $id [.c1 coords $tag]
                 .c1 itemconfigure $id -tags [list $card $newCol card]
                 checkGameStatus
                 return
             }
         }
     }
     .c1 coords $id $::data(orgCoords)
 }

 proc getNextCard {} {
    set cardIdx [random [llength $::data(cards)]]
    set card [lindex $::data(cards) $cardIdx]
    set ::data(cards) [lreplace $::data(cards) $cardIdx $cardIdx]
    return $card
 }
 ...
 proc about {} {
     tk_messageBox \
         ...
         -message "$::data(title)\n\nby Jeff Godfrey, March 2004"
 }
 proc help {} {
     catch {destroy .help}
     toplevel .help
     wm title .help "$::data(title) Help"

     set t [text .help.t -relief raised -wrap word -width 70 -height 23 \
     ...
     $t insert end "$::data(title)\n" title
     $t insert end "by Jeff Godfrey\n" title2
     $t insert end "(beta testing by Taylor Godfrey)\n\n" nc

     set m "Finish the game with only the 4 aces remaining - one in each of the\
            four available columns.\n\n"
     $t insert end "Objective\n" bullet $m n

     set m "The game starts with 4 cards being dealt. Play continues by moving\
            cards, removing cards, and dealing new rows until there are no more\
            cards to deal. If the game ends with the 4 aces displayed in each of\
            the 4 columns, you win. Otherwise, you lose.\n\n"
     $t insert end "Playing\n" bullet $m n

     set m "You can only move or remove the top card in each column. These are\
            the 'cards in play'.   If a card is removed from a column, the newly\
            revealed card in the column is now 'in play'.\n\n"
     $t insert end "Cards in Play\n" bullet $m n

     set m "A card in play can be removed only when its suit matches the suit of\
            another card in play, and its value is less than the value of the\
            other card.  To remove a card, double-click it.\n\n"
     $t insert end "Removing a Card\n" bullet $m n

     set m "A card can only be moved into a empty column.  To move a card, just\
            drag it to a blank column (the card must overlap the column\
            marker, and only the column marker, when it is released).\n\n"
     $t insert end "Moving a Card\n" bullet $m n

     set m "A new row can be dealt at any time by clicking on the deck.\n\n"
     $t insert end "Dealing a New Row\n" bullet $m n

     set m "If menu item \"Game | Show Deal Hints\" is selected, the program\
            will monitor the current game for valid moves.  Once it detects\
            that a card can no longer can be removed, it will draw a red\
            border around the dealer's deck, indicating that you either need\
            to deal more cards or potentially move an existing card to an\
            empty column.  This option is OFF by default.\n\n"
     $t insert end "Showing Deal Hints\n" bullet $m n
